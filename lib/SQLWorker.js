...
import sqlShared, { isSqliteConnectionString } from './sql/shared.js';
import { buildCreateTable, buildAlterTable } from './sql/sqliteDDL.js';
import { sqlIndexName } from './sql/sqlIndexName.js';
import { ObjectError, bool, parseRegExp, getVersionedUUID } from './utilities.js';
...
function dialectFromConnectionString(connectionString) {
...
function Worker(config = {}) {
  ...
  this.accountName = config.accountName || config.name || null;
  ...
  // Optional override for identifier_store_kind (normally read from setting table)
  if (config.identifier_store_kind != null) {
    this.identifier_store_kind = config.identifier_store_kind;
  }
  // Build-time plugin registry (lib/pluginRegistry.js); else the process default.
  if (config.plugins) this.plugins = config.plugins;
  if (config.d1) {
    this.d1 = config.d1;
    ...
    this.dialect = config.dialect || dialectFromConnectionString(this.auth.database_connection);
  } else {
    const resolvedAccount =
      typeof Worker.resolveAccount === 'function' ? Worker.resolveAccount(config.accountId) : null;
    const auth =
      config.auth?.database_connection
        ? { ...config.auth }
        : resolvedAccount?.auth?.database_connection
          ? { ...resolvedAccount.auth }
          : typeof Worker.resolveAccountAuth === 'function'
            ? Worker.resolveAccountAuth(config.accountId)
            : null;
    if (!auth?.database_connection) {
      throw new Error('SQLWorker requires one of: d1 (Cloudflare binding), knex, or auth.database_connection');
    }
    this.auth = { ...auth };
    if (!this.accountName && resolvedAccount?.name) {
      this.accountName = resolvedAccount.name;
    }
    this.dialect = dialectFromConnectionString(this.auth.database_connection);
...
Worker.prototype.destroyAll = Worker.prototype.destroy;
...
function normalizeCreateApiKeyOptions(opts = {}) {
  let { scopes } = opts;
  if (typeof scopes === 'string') scopes = scopes.split(',');
  return {
    name: opts.name || '',
    scopes,
    defaultRoleId: opts.defaultRoleId ?? opts.default_role_id ?? opts['default-role-id'] ?? null,
    expiresAt: opts.expiresAt ?? opts.expires_at ?? opts['expires-at'] ?? null,
    key: opts.key || null
  };
}

function normalizeApiKeyIdOptions(opts = {}) {
  return { id: opts.id || opts.key_id || null };
}

function normalizeListApiKeyOptions(opts = {}) {
  return {
    includeInactive: opts.includeInactive ?? opts.include_inactive ?? true,
    name: opts.name || null,
    id: opts.id || opts.key_id || null,
    active: opts.active
  };
}

function normalizeUpdateApiKeyOptions(opts = {}) {
  let { scopes } = opts;
  if (typeof scopes === 'string') scopes = scopes.split(',');
  const out = {
    id: opts.id || opts.key_id || null
  ...
  if (opts.defaultRoleId !== undefined || opts.default_role_id !== undefined) {
    out.defaultRoleId = opts.defaultRoleId ?? opts.default_role_id;
  }
  if (opts.expiresAt !== undefined || opts.expires_at !== undefined) {
    out.expiresAt = opts.expiresAt ?? opts.expires_at;
  }
  if (opts.active !== undefined) out.active = opts.active;
  return out;
}

async function apiKeyStore(worker) {
  const store = new SqlApiKeyStore({ worker });
  await store.deploy();
  return store;
}

/* Deploy api_key if missing, then SqlApiKeyStore.create. Plaintext key once. */
Worker.prototype.createApiKey = async function (opts = {}) {
  return (await apiKeyStore(this)).create(normalizeCreateApiKeyOptions(opts));
};
Worker.prototype.createApiKey.metadata = {
  description:
    'Deploy the api_key table if missing, then create a hashed Engine9 API key. The plaintext key is returned once.',
  options: {
    name: { description: 'Human-readable key name' },
    scopes: {
      required: true,
      description: 'Comma-separated or array of scopes (e.g. tasks:read,tasks:schedule or admin)'
    },
    defaultRoleId: { description: 'Optional default role (segment UUID)' },
    expiresAt: { description: 'Optional expiration datetime' }
  }
};

Worker.prototype.listApiKeys = async function (opts = {}) {
  return (await apiKeyStore(this)).list(normalizeListApiKeyOptions(opts));
};
Worker.prototype.listApiKeys.metadata = {
  description: 'List API keys for this account (no plaintext, no key_hash).',
  options: {
    includeInactive: { description: 'Include revoked keys (default true)' },
    name: { description: 'Optional exact name filter' },
    id: { description: 'Optional key id filter' },
    active: { description: 'Optional active flag filter' }
  }
};

Worker.prototype.getApiKey = async function (opts = {}) {
  return (await apiKeyStore(this)).get(normalizeApiKeyIdOptions(opts));
};
Worker.prototype.getApiKey.metadata = {
  description: 'Load one API key record by id (no plaintext).',
  options: {
    id: { required: true, description: 'api_key.id UUID' }
  }
};

Worker.prototype.updateApiKey = async function (opts = {}) {
  return (await apiKeyStore(this)).update(normalizeUpdateApiKeyOptions(opts));
};
Worker.prototype.updateApiKey.metadata = {
  description: 'Update key name, scopes, default role, expiration, or active flag without rotating the secret.',
  options: {
    id: { required: true, description: 'api_key.id UUID' },
    name: { description: 'Human-readable key name' },
    scopes: { description: 'Replacement scopes list (comma-separated or array)' },
    defaultRoleId: { description: 'Default role (segment UUID), or empty to clear' },
    expiresAt: { description: 'Expiration datetime, or empty to clear' },
    active: { description: 'true to enable, false to disable' }
  }
};

Worker.prototype.revokeApiKey = async function (opts = {}) {
  return (await apiKeyStore(this)).revoke(normalizeApiKeyIdOptions(opts));
};
Worker.prototype.revokeApiKey.metadata = {
  description: 'Deactivate an API key. The secret cannot be recovered.',
  options: {
    id: { required: true, description: 'api_key.id UUID' }
  }
};

Worker.prototype.rotateApiKey = async function (opts = {}) {
  const store = await apiKeyStore(this);
  const { id } = normalizeApiKeyIdOptions(opts);
  const created = normalizeCreateApiKeyOptions(opts);
  return store.rotate({
    id,
    name: opts.name !== undefined ? created.name : undefined,
    scopes: opts.scopes !== undefined ? created.scopes : undefined,
    defaultRoleId:
      opts.defaultRoleId !== undefined || opts.default_role_id !== undefined
        ? created.defaultRoleId
        : undefined,
    expiresAt: opts.expiresAt !== undefined || opts.expires_at !== undefined ? created.expiresAt : undefined
  });
};
Worker.prototype.rotateApiKey.metadata = {
  description:
    'Create a new key (copying metadata when omitted), revoke the old one, and return the new plaintext once.',
  options: {
    id: { required: true, description: 'api_key.id of the key to revoke' },
    name: { description: 'Optional new name' },
    scopes: { description: 'Optional new scopes' },
    defaultRoleId: { description: 'Optional new default role' },
    expiresAt: { description: 'Optional new expiration' }
  }
};
...
Worker.prototype.tables = async function (options = {}) {
  ...
  if (this.dialect?.introspection?.tableNames) {
    ...
    // MySQL/MariaDB: honor type so callers like truncate({ type: 'table' }) do not
    // attempt TRUNCATE on views (*_summary and similar).
...
Worker.prototype.createTable = async function (opts) {
  ...
  await knex.schema.createTable(table, (t) => {
    this.dialect.applyTableCharacterSet?.(t, opts);
    columns.forEach((c) => {
    ...
    (indexes || []).forEach((x) => {
      ...
      const indexName = sqlIndexName(table, cols, { unique: Boolean(x.unique || x.primary), name: x.name });
      if (x.primary) {
        const autoIncrement = columns.find((c) => c.auto_increment);
        if (!autoIncrement || cols.join(',') !== autoIncrement.name) t.primary(cols, indexName);
      } else if (x.unique) t.unique(cols, indexName);
      else t.index(cols, indexName);
    });
  });
  return { created: true, table };
};
...
Worker.prototype.alterTable = async function (opts) {
  ...
  await knex.schema.alterTable(table, (t) => {
    ...
    (indexes || []).forEach((x) => {
      const cols = typeof x.columns === 'string' ? x.columns.split(',').map((c) => c.trim()) : x.columns;
      const indexName = sqlIndexName(table, cols, { unique: Boolean(x.unique || x.primary), name: x.name });
      if (x.primary) t.primary(cols, indexName);
      else if (x.unique) t.unique(cols, indexName);
      else t.index(cols, indexName);
    });
  });
...
Worker.prototype.insertArray = async function ({ table, array, upsert = false, batchSize = 300 }) {
  ...
  // Same as upsertArray / server insertFromStream: omitted id_uuid PKs must be filled
  // before INSERT (MySQL uuid columns have no DEFAULT under strict mode).
  const idUuidColumns = desc.columns.filter((c) => c.type === 'id_uuid');
  if (idUuidColumns.length > 0) {
    for (const o of array) {
      for (const col of idUuidColumns) {
        if (o[col.name] === undefined || o[col.name] === null || o[col.name] === '') {
          o[col.name] = getVersionedUUID();
        }
      }
    }
  }
...
Worker.prototype.insertOne = async function ({ table, row, data }) {
  row = row || data;
  if (!row || typeof row !== 'object') throw new Error('insertOne requires row (or data)');
  const desc = await this.describe({ table });
  const ignore = ['created_at', 'modified_at'];
  // Assign omitted id_uuid primary keys (same as insertArray / upsertArray)
  for (const col of desc.columns) {
    if (col.type !== 'id_uuid') continue;
    if (row[col.name] === undefined || row[col.name] === null || row[col.name] === '') {
      row[col.name] = getVersionedUUID();
    }
  }
...
export { isSqliteConnectionString };
