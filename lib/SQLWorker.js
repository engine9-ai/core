...
function normalizeCreateApiKeyOptions(opts = {}) {
  ...
  return {
    name: opts.name || '',
    scopes,
    defaultRoleId: opts.defaultRoleId ?? opts['default-role-id'] ?? null,
    expiresAt: opts.expiresAt ?? opts['expires-at'] ?? null,
    key: opts.key || null
  };
}
...
Worker.prototype.createApiKey = async function (opts = {}) {
  const store = new SqlApiKeyStore({ worker: this });
  await store.deploy();
  return store.create(normalizeCreateApiKeyOptions(opts));
};
