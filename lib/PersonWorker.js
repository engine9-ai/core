/*
  ...
  The interface transform modules (person, person_email, person_phone,
  person_remote) are imported from @engine9/interfaces. Optional interfaces
  such as person_hash stay in the registry so extraTransforms can resolve them,
  but are not in the default inbound chain.
*/
...
import { buildInboundTransforms, extendWithPersonCustomUpserts } from './peoplePipeline/getInboundTransforms.js';
...
const TRANSFORM_REGISTRY = {
  ...
  '@engine9/interfaces/person_hash:transforms:id': personHashId,
  '@engine9/interfaces/person_hash:transforms:upsert': personHashUpsert,
...
/* Same transform chain as loadPeople, plus person_custom upserts when installed */
Worker.prototype.getInboundTransforms = async function (options) {
  return buildInboundTransforms(this, options, {
    ...
    extendBeforeUpserts: extendWithPersonCustomUpserts
  });
};
