...
import { getInputUUID, TIMELINE_ENTRY_TYPES } from './ids.js';
import {
  bool,
  parseDate,
  getStringArray,
...
import { uuidIsValid } from './ids.js';
...
function personCreatedAtFromRow(row) {
...
/*
  Assign person_ids to a batch by looking up identifiers in person_identifier,
  creating new person rows for unmatched records, and inserting the new
  identifiers.  Same logic as the server PersonWorker.assignIdsBlocking.
*/
Worker.prototype.assignIdsBlocking = async function (opts) {
  const { batch } = opts;
  const doNotUpsert = bool(opts.doNotUpsert, false);
  batch.forEach((item) => {
  ...
  // Full list of all identifiers, and the objects they're referenced in
  ...
  const tempIdLookup = {};
  // Assign temp ids -- multiple entries in this batch could share an identifier
  batch.forEach((item) => {
    if (!item.person_id) {
  ...
  let existingIds = [];
  if (idArray.length > 0) {
    ({ data: existingIds } = await this.query({
      sql: `select id_value,id_type,person_id from person_identifier where id_value in (${idArray.map(() => '?').join(',')})`,
      values: idArray
    }));
  }
  const existsAlreadyInTableIdLookup = {};
  // Prefer remote_person_id matches over other id types
  existingIds
  ...
  /* Deduplicated list of items that still need an ID */
  ...
  if (tempIds.length > 0 && !doNotUpsert) {
    // Insert person rows one at a time so each new id is exact across
    // MySQL/SQLite/D1 (batch LAST_INSERT_ID semantics differ per engine)
    for (const t of tempIds) {
      const row = {};
      const createdAt = personCreatedAtFromRow(lookupByTempId[t]);
      if (createdAt) row.created_at = createdAt;
      const { id } = await this.insertOne({ table: 'person', row });
      tempIdToPersonIdLookup[t] = id;
    }
  }
  // Assign person_ids and build the person_identifier insert
  ...
  batch.forEach((item) => {
    ...
    (item.identifiers || []).forEach((id) => {
      ...
      personIdentifersToInsert[id.value] = {
        person_id: item.person_id,
        // input_id is the connection this record first came from
        source_input_id: item.input_id,
        id_type: id.type,
        id_value: id.value
      };
  ...
  if (identifiersToInsert.length > 0 && !doNotUpsert) {
    await this.insertArray({ table: 'person_identifier', array: identifiersToInsert });
  }
  return batch;
};
...
Worker.prototype.appendPersonId = async function ({ batch, inputId, options = {} }) {
  ...
  await this.assignIdsBlocking({ batch, inputId, doNotUpsert });
  return batch;
};
  ...
  await Promise.all(
    Object.keys(bindings).map(async (name) => {
      ...
      if (binding.path === 'sql.query') {
        ...
        const { data } = await this.query({
          sql: `select * from ${this.escapeTable(table)} where ${this.escapeColumn(lookup.tableLookupField)} in (${[...values].map(() => '?').join(',')})`,
          values: [...values]
        });
...
Worker.prototype.getInboundTransforms = async function (options) {
  ...
  if (options.extraTransforms) {
    ...
    Object.entries(o).forEach(([key, transforms]) => {
      ...
      let arr = transforms;
      if (typeof arr === 'string') arr = getStringArray(arr).map((p) => ({ path: p, options: {} }));
      if (!Array.isArray(arr)) arr = [arr];
      extraTransforms[key].push(...arr);
    });
  }
  let transforms = extraTransforms.beforeAll.concat([
    { path: '@engine9/interfaces/person:transforms:normalizeFieldNames', options: {} },
    { path: '@engine9/interfaces/person_remote:transforms:id', options: {} },
    { path: '@engine9/interfaces/person_email:transforms:id', options: {} },
    { path: '@engine9/interfaces/person_phone:transforms:id', options: {} },
    {
  ...
  if (doNotUpsert) {
    debug('doNotUpsert was specified, skipping upserts');
  } else {
    transforms = transforms.concat([
      { path: '@engine9/interfaces/person:transforms:upsert', options: {} },
      { path: '@engine9/interfaces/person_remote:transforms:upsert', options: {} },
      { path: '@engine9/interfaces/person_email:transforms:upsert', options: {} },
      { path: '@engine9/interfaces/person_phone:transforms:upsert', options: {} }
    ]);
  }
  transforms = transforms.concat(extraTransforms.afterAll);
  return transforms;
};
...
Worker.prototype.processPeople = async function (opts) {
  ...
  for (const transformConfig of transformConfigArray) {
    const { transform, bindings, options, path } = await this.resolveTransform(transformConfig);
    ...
    if (output.batch) {
      const tablesToUpsert = batch.tablesToUpsert;
      batch = output.batch;
      batch.tablesToUpsert = tablesToUpsert;
    }
    executionStats[path] = executionStats[path] || { time: 0, records: 0 };
    executionStats[path].time += Date.now() - start;
    executionStats[path].records += batch.length;
  }
  const summary = {
