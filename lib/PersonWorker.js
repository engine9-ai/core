/*
  Engine9 person worker.

  Runs the inbound person pipeline (identifier extraction, input resolution,
  person id assignment, source code resolution, table upserts). Core deployments
  use in-memory processPeople; PluginWorker.install / installStandard live on
  this class via inheritance.

  The interface transform modules are imported statically from
  @engine9/interfaces so the client bundles (Cloudflare Workers cannot import
  unknown packages at runtime). Which of them run is decided per account by the
  inbound weaver: a plugin's transforms are in the chain only when that plugin
  is installed. See peoplePipeline/getInboundTransforms.js.
*/
import debug$0 from 'debug';
import JSON5 from 'json5';
import { cleanUnicodeValues } from '@engine9/input-tools/checkUnicode.js';
import PluginWorker from './PluginWorker.js';
import {
  relativeDate,
  getInputUUID,
...
import { buildInboundTransforms, describeInboundTransforms } from './peoplePipeline/getInboundTransforms.js';
...
import personHashId from '@engine9/interfaces/person_hash/transforms/inbound/extract_identifiers.js';
import personHashUpsert from '@engine9/interfaces/person_hash/transforms/inbound/upsert_tables.js';
...
const TRANSFORM_REGISTRY = {
  ...
  '@engine9/interfaces/person:transforms:upsertPerson': personUpsert,
  '@engine9/interfaces/person_remote:transforms:extractRemotePersonIds': personRemoteId,
  '@engine9/interfaces/person_remote:transforms:upsertPersonRemote': personRemoteUpsert,
  '@engine9/interfaces/person_email:transforms:extractEmailHashes': personEmailId,
  '@engine9/interfaces/person_email:transforms:upsertPersonEmail': personEmailUpsert,
  '@engine9/interfaces/person_phone:transforms:extractPhoneHashes': personPhoneId,
  '@engine9/interfaces/person_phone:transforms:upsertPersonPhone': personPhoneUpsert,
  '@engine9/interfaces/person_hash:transforms:extractContactHashes': personHashId,
  '@engine9/interfaces/person_hash:transforms:upsertPersonHash': personHashUpsert,
  '@engine9/interfaces/person_address:transforms:normalize': personAddressNormalize,
  '@engine9/interfaces/person_address:transforms:upsertPersonAddress': personAddressUpsert
};

/** Plugin packages this client can execute (derived from the registry, not a second list) */
export const KNOWN_TRANSFORM_PLUGIN_PATHS = [
  ...new Set(Object.keys(TRANSFORM_REGISTRY).map((p) => p.split(':')[0]))
];

function Worker(config) {
  PluginWorker.call(this, config);
}
Worker.prototype = Object.create(PluginWorker.prototype);
...
/*
  Extract delegate identifiers (this Domain's Pseudonym, and the Profile
  subject when one was shared) so logins recognize/dedupe through the normal
  person identifier pipeline (person_id_delegate on SQLite/D1,
  person_identifier id_type 'delegate' on MySQL).
*/
Worker.prototype.extractDelegateIdentifiers = async function ({ batch }) {
  batch.forEach((o) => {
    o.identifiers = o.identifiers || [];
    for (const value of [o.delegate_id, o.delegate_subject]) {
...
Worker.prototype.validateSourceCodeAscii = async function ({ batch, options = {} }) {
  ...
  const { count, samples, failures } = cleanUnicodeValues(batch, {
    field: 'source_code',
    maxSamples: 5,
    maxLength: 180
  });
  if (count > 0) {
    info(
      `${tableRef} cleaned ${count} invalid source_code value(s) (length/unicode). Samples: ${JSON.stringify(samples)}`
    );
  }
  if (failures.length === 0) return;
  throw new Error(
    `${tableRef} source_code must be printable ASCII only (U+0020 through U+007E), max 180 characters after cleaning. Invalid on ${failures.length} row(s). Samples: ${JSON.stringify(failures.slice(0, 5))}`
  );
};
...
/*
  Weave the inbound chain from installed plugins (same as server loadPeople),
  plus the client-only delegate identifier step. Pass { describe: true } to get
  the human-readable slot/source/path listing instead of the step array.
*/
Worker.prototype.getInboundTransforms = async function (options = {}) {
  const { describe, ...pipelineOptions } = options;
  const steps = await buildInboundTransforms(this, pipelineOptions, {
    beforeIdentityTransforms: [{ path: 'person.extractDelegateIdentifiers', options: {} }],
    knownPluginPaths: KNOWN_TRANSFORM_PLUGIN_PATHS
  });
  return describe ? describeInboundTransforms(steps) : steps;
};
...
Worker.prototype.processPeople = async function (opts) {
  ...
  if (!doNotUpsert) {
    transformConfigArray.push({ path: 'sql.tables.upsert', slot: 'upsert', source: 'core' });
  }
  debug('inbound pipeline:\n%s', describeInboundTransforms(transformConfigArray));
