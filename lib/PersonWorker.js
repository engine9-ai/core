...
/*
  Extract delegate identifiers (the Domain UNID, and the Domain UNID it
  merged from when a browser's UNID was merged at login) so logins
  recognize/dedupe through the normal person identifier pipeline (person_id_delegate on SQLite/D1,
...
Worker.prototype.extractDelegateIdentifiers = async function ({ batch }) {
  batch.forEach((o) => {
    ...
    for (const value of [o.delegate_id, o.delegate_merged_from]) {
...
Worker.prototype.resolveTransform = async function (o) {
  ...
  // <plugin path>:transforms:<name>, looked up in the build's plugin registry.
  const [pluginPath, kind, name] = String(path).replace(/^local\$/, '').split(':');
  if (!pluginPath || kind !== 'transforms' || !name) {
    throw new Error(`Invalid transform path ${path}: expected <plugin path>:transforms:<name>`);
  }
  const compiled = await this.compilePlugin({ path: pluginPath });
  const compiledTransform = compiled.transforms?.[name];
  if (!compiledTransform) {
    throw new Error(`Plugin ${pluginPath} has no transform ${name} (from ${path})`);
  }
...
Worker.prototype.getInboundTransforms = async function (options = {}) {
  ...
  const steps = await buildInboundTransforms(this, pipelineOptions, {
    ...
    knownPluginPaths: (await pluginRegistryFor(this)?.paths()) || []
  });
