...
/** Interfaces that may be installed more than once per account (own table prefix per row). */
const NON_UNIQUE_INTERFACE_PATHS = new Set(['@engine9/interfaces/person_custom']);

/**
 * Whether PluginWorker.install should reuse an existing plugin row for this path.
 *
 * Precedence:
 * 1. metadata.unique (or top-level pluginConfiguration.unique) when defined
 * 2. known non-unique interfaces (person_custom)
 * 3. @engine9/interfaces/* default unique (one row per account)
 * 4. options.unique, otherwise false — third-party plugins may install multiple times
 */
function resolvePluginInstallUnique({
  path: pluginPath,
  metadata,
  unique: optionsUnique,
  pluginConfiguration
} = {}) {
  const meta = metadata || pluginConfiguration?.metadata;
  if (meta?.unique !== undefined) return Boolean(meta.unique);
  if (pluginConfiguration?.unique !== undefined) return Boolean(pluginConfiguration.unique);
  const normalized = normalizePluginInstallPath(pluginPath);
  if (NON_UNIQUE_INTERFACE_PATHS.has(normalized)) return false;
  if (normalized.startsWith('@engine9/interfaces/')) return true;
  return Boolean(optionsUnique);
}
...
function omitKeys(source, keys) {
  ...
  return Object.fromEntries(Object.entries(source).filter(([key]) => !omit.has(key)));
}
...
export { NON_UNIQUE_INTERFACE_PATHS };
export { resolvePluginInstallUnique };
export { getInputUUID };
...
export default {
  ...
  FixedQueue,
  getPluginUUID,
  NON_UNIQUE_INTERFACE_PATHS,
  resolvePluginInstallUnique,
  getInputUUID,
  getVersionedUUID,
  getUUIDTimestamp,
  getEntryTypeId,
