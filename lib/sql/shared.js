/*
  ...
  @engine9/server SQLWorker and the @engine9/core SQLWorker use the exact
  same code:
...
import { bool, diffObjectKeysAcrossSamples, getVersionedUUID } from '../utilities.js';

const debug = debug$0('SQLWorker');
const info = debug$0('info:SQLWorker');

function isGeneratedIdColumn(column) {
  return Boolean(column?.auto_increment || column?.type === 'id_uuid');
}
...
export function buildInsertSql(options) {
  ...
  if (bool(upsert, false)) {
    // Do not rewrite generated primary keys on conflict (auto_increment / id_uuid)
    const updateColumns = columns.filter((column) => !isGeneratedIdColumn(column));
    if (updateColumns.length === 0) {
      const n = worker.escapeColumn(columns[0].name);
      sql += ` ${worker.onDuplicate()} ${n}=${n}`;
    } else {
      sql += ` ${worker.onDuplicate()} ${updateColumns
        .map((column) => {
...
export async function upsertArray({ table, array }) {
  ...
  // Assign UUID primary keys when callers omit them (same pattern as insertOne / API POST)
  const idUuidColumns = desc.columns.filter((c) => c.type === 'id_uuid');
  if (idUuidColumns.length > 0) {
    array.forEach((o) => {
      idUuidColumns.forEach((col) => {
...
export function isSqliteConnectionString(connectionString) {
  return typeof connectionString === 'string' && connectionString.indexOf('sqlite://') === 0;
}
...
export function isDuplicateKeyError(err) {
  return (
    ...
    err?.errno === 1062 ||
    (typeof err?.sqlMessage === 'string' && err.sqlMessage.includes('Duplicate entry')) ||
...
/** Missing relation from MySQL, SQLite, Postgres, or engine9 describe(). */
export function isMissingTableError(err) {
  if (!err) return false;
  if (err.does_not_exist === true) return true;
  const code = err.code;
  if (code === 'DOES_NOT_EXIST' || code === 'ER_NO_SUCH_TABLE') return true;
  if (err.errno === 1146) return true;
  const message = String(err.message || err.sqlMessage || '');
  return /no such table|unknown table|doesn't exist|could not find table|relation .+ does not exist/i.test(
    message
  );
}
