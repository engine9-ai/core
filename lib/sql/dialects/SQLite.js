...
import { isDuplicateKeyError } from '../shared.js';
...
export const types = [
  ...
  },
  {
    ...
    // SQLite has no BINARY type; stored as BLOB. Distinct from `hash` (64-char hex).
    type: 'id_u128',
    column_type: 'blob',
    knex_method: 'specificType',
    knex_args: () => ['blob'],
  ...
  {
    // SQLite has no ON UPDATE clause. The one trigger in getPostCreateStatements
    // keeps modified_at current. Do not add triggers for anything else.
    type: 'modified_at',
    column_type: 'datetime',
    default_value: 'current_timestamp()',
    nullable: false,
...
// SQLite is always UTF-8. charset/collation stay undefined so knex and
// CREATE TABLE AS callers skip MySQL CHARSET/COLLATE (see applyTableCharacterSet).
export function getTableCharacterSetSpecification() {
  return '';
}
export function applyTableCharacterSet() {}
...
/* The only trigger in this package. SQLite has no ON UPDATE CURRENT_TIMESTAMP,
   so this emulates the MySQL modified_at column clause. Not a pattern to copy. */
...
export function dialectToStandard(o, defaultColumn) {
  ...
  if (input.column_type === 'binary(16)') {
    input.column_type = 'blob';
  }
