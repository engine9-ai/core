...
/**
 * Attach `settings` from the compiled module, then from a sibling settings.js.
 * Reinstall uses this list to insert missing warehouse `setting` rows.
 */
async function attachCompiledPluginSettings(plugin, { mod, resolvedFsEntry, packagePath } = {}) {
  if (plugin.settings == null && mod?.settings != null) plugin.settings = mod.settings;
  if (plugin.settings != null) return plugin;

  if (resolvedFsEntry) {
    const settingsFile = path.join(path.dirname(resolvedFsEntry), 'settings.js');
    try {
      await fs.promises.access(settingsFile, fs.constants.R_OK);
      const settingsMod = await import(pathToFileURL(settingsFile).href);
      plugin.settings = settingsMod.settings || settingsMod.default?.settings;
    } catch (e) {
      if (e?.code !== 'ERR_MODULE_NOT_FOUND' && e?.code !== 'MODULE_NOT_FOUND' && e?.code !== 'ENOENT') {
        throw e;
      }
    }
    return plugin;
  ...
  if (packagePath?.startsWith('@')) {
    try {
      const settingsMod = await import(`${packagePath}/settings.js`);
      plugin.settings = settingsMod.settings || settingsMod.default?.settings;
    } catch (e) {
...
Worker.prototype.compilePlugin = async function ({ path: pluginPath, source } = {}) {
  ...
  if (typeof resolve === 'function') {
    ...
    await attachCompiledPluginSettings(plugin, { mod, resolvedFsEntry, packagePath });
    return plugin;
  }
  ...
  const plugin = Object.assign({}, mod.default || mod);
  plugin.path = packagePath;
  await attachCompiledPluginSettings(plugin, { mod, packagePath });
  return plugin;
};
