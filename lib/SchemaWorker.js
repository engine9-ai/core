...
/*
  Plugins come from the build-time registry (lib/pluginRegistry.js), never
  from disk. `source` works only with a registry that supports it (the server
  runtime-plugins deployment).
*/
Worker.prototype.compilePlugin = async function ({ path: pluginPath, source } = {}) {
  return compileRegistryPlugin(pluginRegistryFor(this), pluginPath, { source });
};
