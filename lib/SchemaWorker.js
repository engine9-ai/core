...
import { getPluginUUID, getVersionedUUID, resolvePluginInstallUnique } from './utilities.js';
...
Worker.prototype.install = async function (options) {
  ...
  const unique = resolvePluginInstallUnique({ path: pluginPath, unique: options.unique });
  ...
  if (!id && unique && type !== 'local' && !isInterface) {
  ...
  if (unique && installedPlugins.length > 1) {
    throw new Error(
      `Error in plugin table: more than one plugin configured for ${pluginPath} (unique). Keep ${installedPlugins[0].id} and delete the extras.`
    );
  }
