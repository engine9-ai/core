...
import { isSqliteLike } from './sqlHelpers.js';
import {
  identifierMapKey,
  hashIdValueToU128,
...
/**
 * Pick the identifier store for this worker:
 *   - Durable Object namespace (worker.personIds / PERSON_IDS) → DO compact store
 *   - SQLite / D1 → compact person_id_<id_type> tables
 *   - MySQL (and other SQL) → full person_identifier table
 */
export function createDefaultIdentifierStore(worker) {
  ...
  if (isSqliteLike(worker)) {
    return createCompactSqlIdentifierStore(worker);
  }
  return sqlStore.createPersonIdentifierSqlStore(worker);
}
...
export async function assignPersonIds({
  ...
  const store = identifierStore || createDefaultIdentifierStore(worker);
