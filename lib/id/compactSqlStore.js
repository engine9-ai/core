 ...
 *   value      BLOB / BINARY(16) / FixedString(16)  -- standard type id_u128 (128-bit SHA-256 truncate)
...
import { getKnex, isSqliteLike } from './sqlHelpers.js';
...
async function insertIgnoreRows(worker, table, rows) {
  ...
  const chunkSize = 400;
  const sqlite = isSqliteLike(worker);
