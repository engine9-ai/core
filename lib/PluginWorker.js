...
function normalizePluginSettings(settings) {
  ...
  if (Array.isArray(settings)) {
    return settings.filter((row) => row && typeof row === 'object' && row.name).map((row) => ({
      ...row,
...
PluginWorker.prototype.installStandard = async function (opts = {}) {
  const resolvedNow =
    typeof PluginWorker.resolveDefaultStackPath === 'function'
      ...
      : null;
  const pathToInstall =
    opts.path || opts.stack || resolvedNow || this.defaultStackPath || resolveDefaultStackPath(this);
...
PluginWorker.prototype.installStandard.metadata = {
  options: {
    path: {
      description:
        'Stack (or plugin) path to install. Defaults to the account default stack (limited-pii when settings.exclude_pii).'
    }
  }
};
