...
PluginWorker.prototype.installRow = async function installRow(options) {
  ...
  if (standardPluginDefaults[pluginPath]) {
    ...
    // Throws PLUGIN_NOT_IN_BUILD when the plugin was not compiled into this build.
...
PluginWorker.prototype.install.metadata = {
  options: {
    path: {
      description:
        'Plugin package identity (@engine9/interfaces/..., @engine9/plugins/...) or shorthand resolved via listAvailable (e.g. transaction/contact_details, e9email). The plugin must be in this build (e9core build-plugins). Legacy local$ prefix is accepted and stripped. Defaults to the account default stack.'
    },
    source: {
      description:
        'Runtime-plugins deployments only: absolute directory, file, or file: URL to load the module from (unpublished checkout). Identity remains path. Pre-compiled builds reject it.'
    },
