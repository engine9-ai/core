 ...
 *
 * Mutates the same record object references that were passed in. Server idFiles
 * passes a batch into loadPeople and reads person_id (and other enrichments) off
 * those same objects after the stream finishes — shallow-cloning here left the
 * caller's records without person_id while ids were assigned only on copies.
 */
