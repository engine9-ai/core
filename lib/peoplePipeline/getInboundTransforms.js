...
/*
  ...
  the plugin in the build registry. Cached per worker. Returns null when the
  plugin loaded but declares no inbound steps, and undefined when it could not
  be loaded (not in this build, or its module failed).
*/
async function inboundFromPackage(worker, path) {
  if (typeof worker.compilePlugin !== 'function') return undefined;
...
export async function loadInboundPlugins(worker, { knownPluginPaths = [] } = {}) {
  ...
  for (const row of rows) {
    ...
    if (!inbound) {
      if (inbound === undefined && knownPluginPaths.includes(row.path)) {
        throw new Error(
          `Plugin ${row.path} is installed and in this build, but has no inbound snapshot and failed to load. Re-run install (installStandard) to refresh plugin.transforms.`
        );
      }
      continue;
    }
