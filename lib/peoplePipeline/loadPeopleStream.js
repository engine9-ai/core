...
function isImmediatePeopleProgressStep(path) {
  const p = String(path || '');
  return /upsert|appendPersonId|sql\.tables/i.test(p);
}
...
export async function runLoadPeopleStream({
  ...
  const progressHeartbeatMs = Math.max(
    0,
    Number(progressHeartbeatMsOpt ?? progressHeartbeatMsLegacy ?? 5000) || 0
  );
  ...
  // Event-driven progress is throttled and is not called during a blocking
  // transform, so a heartbeat is required or the marker stays stale. Do not
  // emit the initial pipeline-configured zeros — idFiles starts a new
        ...
        try {
          const result = await runPeopleBatchPipeline({
            ...
            onStepStart: ({ path, batchIndex: idx, batchSize }) => {
              ...
              // Emit before a likely-slow step so the UI is not left on an
              // earlier (or zero) snapshot if the transform blocks the event loop.
              if (isImmediatePeopleProgressStep(path)) emitLoadPeopleProgress();
              else reportLoadPeopleProgress();
            },
