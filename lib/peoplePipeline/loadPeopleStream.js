...
import { formatEntryTypeCounts, tallyEntryType } from './entryTypeCounts.js';
...
/** Per-step start/done info logs for the first N records, then only batch sizes. */
export const VERBOSE_PEOPLE_BATCH_LOG_RECORDS = 3000;
/** Max frequency for loadPeople progress (event throttle and heartbeat). */
export const DEFAULT_LOAD_PEOPLE_PROGRESS_MS = 15000;

function shouldLogPeopleBatchDetail(recordsOffset, recordsBeforeBatch) {
  return recordsOffset + recordsBeforeBatch < VERBOSE_PEOPLE_BATCH_LOG_RECORDS;
}

function resolveProgressStartedAt(raw, fallbackMs) {
  const n = Number(raw);
  if (Number.isFinite(n) && n > 0 && n <= fallbackMs) return n;
  return fallbackMs;
}

function formatRecordsPerSecond(count, elapsedMs) {
  if (!(count > 0) || !(elapsedMs > 0)) return '0';
  return ((1000 * count) / elapsedMs).toFixed(1);
}
...
export async function runLoadPeopleStream({
  ...
  const {
    ...
    progress_heartbeat_ms: progressHeartbeatMsOpt,
    progressHeartbeatMs: progressHeartbeatMsLegacy,
    records_offset: recordsOffsetOpt,
    recordsOffset: recordsOffsetLegacy,
    progress_started_at: progressStartedAtOpt,
    progressStartedAt: progressStartedAtLegacy,
    log_entry_type_counts: logEntryTypeCountsOpt,
    logEntryTypeCounts: logEntryTypeCountsLegacy,
    source_label: sourceLabelOpt,
    sourceLabel: sourceLabelLegacy,
  ...
  const progressHeartbeatMs = Math.max(
    0,
    Number(progressHeartbeatMsOpt ?? progressHeartbeatMsLegacy ?? DEFAULT_LOAD_PEOPLE_PROGRESS_MS) || 0
  );
  const progressThrottleMs = progressHeartbeatMs > 0 ? progressHeartbeatMs : DEFAULT_LOAD_PEOPLE_PROGRESS_MS;
  const recordsOffset = Math.max(0, Number(recordsOffsetOpt ?? recordsOffsetLegacy ?? 0) || 0);
  ...
  const start = resolveProgressStartedAt(
    progressStartedAtOpt ?? progressStartedAtLegacy,
    callStartedAt
  );
  const logEntryTypeCounts = Boolean(logEntryTypeCountsOpt ?? logEntryTypeCountsLegacy);
  const sourceLabel = sourceLabelOpt || sourceLabelLegacy || filenameOpt || '';
  ...
  function formatLoadPeopleProgressMessage() {
    ...
    const completed = recordsOffset + summary.records;
    const sourced = recordsOffset + summary.sourceRecords;
    const completedRate = formatRecordsPerSecond(completed, elapsedMs);
    const sourceRate = formatRecordsPerSecond(sourced, elapsedMs);
    ...
    const where = sourceLabel || loadContext.sourceTable;
    ...
    return `loadPeople${wherePart}: ${completed} records completed (${completedRate}/s)${pendingPart}, ${sourced} sourced (${sourceRate}/s), sql batches ${sqlBatchIndex}, stage: ${stageDetail}`;
  }

  function formatLoadPeopleEntryTypeMessage(suffix = '') {
    const labelPart = sourceLabel ? ` ${sourceLabel}` : '';
    const countsPart = formatEntryTypeCounts(entryTypeCounts) || '(none)=0';
    return `loadPeople entry_types${labelPart} ${countsPart} records=${summary.sourceRecords}${suffix}`;
  }

  function emitLoadPeopleEntryTypeCounts(suffix = '') {
    if (!logEntryTypeCounts) return;
    info(formatLoadPeopleEntryTypeMessage(suffix));
  }

  function emitLoadPeopleProgress(sample, suffix = '') {
    const message = `${formatLoadPeopleProgressMessage()}${suffix}`;
    debug(`Progress: ${message}`, sample ? JSON5.stringify(sample) : '');
    progress(message);
  }

  const reportLoadPeopleProgress = throttleProgress(progressThrottleMs, (sample) => {
    emitLoadPeopleProgress(sample);
  });
  const reportLoadPeopleEntryTypeCounts = throttleProgress(progressThrottleMs, () => {
    emitLoadPeopleEntryTypeCounts();
  });
  ...
  function isVerboseBatchLog() {
    // Source can run ahead of processing; use completed records so the
    // first few thousand processed still get per-step start/done logs.
    return shouldLogPeopleBatchDetail(recordsOffset, summary.records);
  }

  function logSqlBatchFormed(message, batchLength) {
    if (shouldLogPeopleBatchDetail(recordsOffset, summary.sourceRecords - batchLength)) {
      info(message, JSON5.stringify(loadContext));
    } else {
      info(message);
    }
  }
  ...
  // Event-driven progress is throttled and does not run during a blocking
  // transform, so a heartbeat (same interval) is required or the marker stays
  // stale. Skip the initial pipeline-configured zeros — idFiles starts a new
  // loadPeople per parquet batch, and that first emit reset the UI to 0.
  const progressHeartbeatTimer =
    progressHeartbeatMs > 0
      ? setInterval(() => {
          if (summary.sourceRecords === 0 && sqlBatchIndex === 0) return;
          reportLoadPeopleProgress();
          reportLoadPeopleEntryTypeCounts();
        }, progressHeartbeatMs)
      : null;
  if (progressHeartbeatTimer?.unref) progressHeartbeatTimer.unref();
  ...
  const pipelineIdleTimer = setInterval(() => {
    ...
    emitLoadPeopleProgress(undefined, ` (idle ${Math.round(idleMs / 1000)}s)`);
  ...
  const transforms = [
    ...
    new Transform({
      ...
      async transform(o, enc, cb) {
        ...
        tallyEntryType(entryTypeCounts, o);
        ...
        try {
          const result = await runPeopleBatchPipeline({
            ...
            onPhase: ({ path, phase }) => {
              ...
              notePipelineActivity(phase, { path, batchIndex });
            },
            onStepStart: ({ path, batchIndex: idx, batchSize }) => {
              batchIndexes[path] = idx;
              inFlightEntry.path = path;
              inFlightEntry.phase = 'start';
              notePipelineActivity('step-start', { path, batchIndex: idx });
              if (isVerboseBatchLog()) {
                info(
  ...
  try {
    ...
    reportLoadPeopleProgress.cancel?.();
    reportLoadPeopleEntryTypeCounts.cancel?.();
    batchStallWatcher.clearAll?.();
    emitLoadPeopleProgress(undefined, ' (complete)');
    emitLoadPeopleEntryTypeCounts(' (complete)');
