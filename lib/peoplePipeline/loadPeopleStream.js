...
export async function runLoadPeopleStream({
  ...
  const transforms = [
    ...
    new Transform({
      objectMode: true,
      highWaterMark: pipelineHighWaterMark,
  ...
  for (const transformConfig of transformConfigArray) {
    const pathHint = transformConfig.path || transformConfig;
    const transformObject = new Transform({
      ...
      async transform(batch, enc, cb) {
        ...
        const inFlightEntry = { path: pathHint, batchIndex, startedAt: batchStartedAt, phase: 'bindings' };
